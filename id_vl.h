...
void VL_DrawPicBare             (int x, int y, uint8_t *pic, int width, int height);
void VL_MemToLatch              (uint8_t *source, int width, int height,
                                    SDL_Surface *destSurface, int x, int y);
void VL_ScreenToScreen          (SDL_Surface *source, SDL_Surface *dest);
void VL_MemToScreenScaledCoord  (uint8_t *source, int width, int height, int scx, int scy);
void VL_MemToScreenScaledCoord  (uint8_t *source, int origwidth, int origheight, int srcx, int srcy,
                                    int destx, int desty, int width, int height);

void inline VL_MemToScreen (uint8_t *source, int width, int height, int x, int y)
{
    VL_MemToScreenScaledCoord(source, width, height, scaleFactor*x, scaleFactor*y);
}

void VL_MaskedToScreen (uint8_t *source, int width, int height, int x, int y);
